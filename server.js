...
async function getPubUrl(lang, pub, issue) {
  ...
  try {
    ...
    if (!fileUrl) {
      if (!lang || !pub) {
        return res.status(400).json({
          ...
          example: '/api/parse?url=https://example.com/file.jwpub&date=2024-11-04 or /api/parse?lang=E&pub=mwb&issue=202411&date=2024-11-04',
        });
      }

      const langStr = lang || 'E';
      const pubStr = pub || 'mwb';
      let issueStr;
      ...
      if (issue) {
        ...
        if (pubStr === 'mwb') {
          ...
          return res.status(400).json({ 
            error: "Invalid pub type. Use 'mwb' or 'w'" 
          });
        }
      }
    ...
    if (!fileUrl) {
      return res.status(400).json({
        ...
        example: '/api/parse?url=https://example.com/file.jwpub&date=2024-11-04',
      });
    }
